...
import ModalPredlogPlanine from "./ModalPredlogPlanine"
...
function ProfilObicnogKorisnika(props) {
    console.log("CRTAM ProfilObicnogKorisnika?");


    const [modalPromenaRole, setModalPromenaRole]=React.useState(false);
    const [loading, setLoading] = React.useState([]);
    const [modalPredlog, setModalPredlog] = React.useState(false);
    const [pretDogadjaji, setPretDogadjaji] = React.useState([]);
    ...
    React.useEffect(() => {
        setLoading(true);
        console.log("ID korisnika # "+props.id);
        const getDg = async () => {
    ...
    //PreuzmiPrethodneDogadjajeKorisnika/{korisnikId}
    React.useEffect(() => {
        setLoading(true);
        const getpretDogKor = async () => {
            const res = await fetch("https://localhost:5001/Dogadjaj/PreuzmiPrethodneDogadjajeKorisnika/" + props.id);
            const getdata = await res.json();
            console.log("@#@#######@#@##@#@#@#@#@");
            console.log(getdata);
            setPretDogadjaji(getdata);
            setLoading(false);
        }
        getpretDogKor();
    }, [props.id]);
    ...
    function gasiModalPredlog() {
        setModalPredlog(false);
    }
    ...
    return (
        <div>
            ...
            {(modalPredlog && <ModalPredlogPlanine show={modalPredlog} funkcijaGasi={gasiModalPredlog} />)}
            ...
            <h6 className="mt-4 text">Ime i prezime:</h6>
              ...
              <div className="text-right dugme">
                <button className="btn btn-style btn-secondary" onClick={izmenaProfila}>Izmeni</button>
                                        ...
                                        <button className="dugme btn btn-style btn-secondary" onClick={() => setModalPredlog(true)}>Predlozi planinu</button>
            ...
            <div>
              ...
              <div>
                {
                    pretDogadjaji.map((element, index) => {
                        console.log(element);
                        return (
                            <div>
                                <NavItem>
...
export default ProfilObicnogKorisnika;